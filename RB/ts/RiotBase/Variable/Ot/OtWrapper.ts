...
class OtWrapper extends Rp {
    ...
    self_on_change( callback ) : PatchId {
        let pid = new PatchId( new DevId, on_change_cur_cb_id += 1 );
        this.on_change_callbacks[ pid.toString() ] = callback;
        return pid;
    }

    self_rm_on_change( callback_id : PatchId ) {
    ...
    sig_change() {
        ...
        for( let cb in this.on_change_callbacks )
            RegCallbacks.add( this.on_change_callbacks[ cb ] );
    }
    ...
    on_change_callbacks : { [key:number]: (() => void) };
    on_change_par       : Array<{ par: Variable, arg: any }>;
    last_ops            : BinaryWriter;
    usr_rights          : { [usr_id:string]: number }; // usr_id -> unsigned (right flags)
    grp_rights          : { [grp_id:string]: number }; // grp_id -> unsigned (right flags)
    usr_id              : UsrId; // creator (who has all the right at the beginning)
}

// Methods that will work with an OtWrapper
for( let n of [ "self_on_change", "self_on_change_par", "pull_ot_changes", "new_ot_merger", "get_possible_rights", "get_groups_in_acl", "get_users_in_acl",
                "get_grp_rights", "get_usr_rights", "add_grp_right", "add_usr_right", "rem_grp_right", "rem_usr_right" ] ) {
    Method.methods[ n ].add_surdef( 0.5, [ "make_ot_wrapper" ], ( ...args ) => {
        args[ 0 ].rp = Method.methods["make_ot_wrapper"].call_1( args[ 0 ] );
        return Method.methods[ n ].call_1.apply( Method.methods[ n ], args );
    } );
}

// for( let n of [ "self_ot_merge_b" ] ) {
//     Method.methods[ n ].add_surdef( 1 + 0.1 / ( 1 + 5 ), [ "make_ot_wrapper", () => true ], ( ...args ) => {
//         args[ 0 ].rp = Method.methods["make_ot_wrapper"].call_1( args[ 0 ] );
//         return Method.methods[ n ].call_2.apply( Method.methods[ n ], args );
//     } );
// }
...
import RegCallbacks from "../Core/RegCallbacks";
