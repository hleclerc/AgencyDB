...
class OtWrapperString extends OtWrapper {
    ...
    static op_insert = {
        ...
        apply_op( rp: OtWrapperString, aod, pos : number, sup: string ) : void {
            ...
            Method.methods["self_insert_bb"].call_3( rp.val, new Number( pos ), new String( sup ) );
            rp.sig_change()
        },
    ...
    static op_remove = {
        ...
        apply_op( rp: OtWrapperString, aod, pos : number, len: number ) : void {
            ...
            Method.methods["self_remove_bb"].call_3( rp.val, new Number( pos ), new Number( len ) );
            rp.sig_change()
        },
    ...
    _self_insert( pos : number, sup : string, usr_id = new UsrId() ) : boolean {
        if ( sup.length && OtWrapperString.op_insert.right( this, this.usr_right( usr_id ), pos, sup ) ) {
            OtWrapperString.reg_op( this.last_ops, OtWrapperString.op_insert, pos, sup );
            OtWrapperString.op_insert.apply_op( this, { usr: usr_id, type: 0 }, pos, sup );
            return true;
        }
        return false;
    }
    ...
    new_cursors: Array<Cursor>;
    mod_cursors: Array<Cursor>;
    rem_cursors: Array<Cursor>;
}
...
Method.methods["self_set_o"].add_surdef( 2, [ OtWrapperString, "to_String" ], function( str : String, sup ) : boolean {
...
Method.methods["self_set_b"].add_surdef( 2, [ OtWrapperString, "to_String" ], function( str : String, sup ) : boolean {
    if ( ! str.equ_immediate( sup ) ) {
        str.clear();
        str.insert( 0, sup.toString() );
        return true;
...
Method.methods["self_add_b"].add_surdef( 2, [ OtWrapperString, "to_String" ], function( str, sup ) : boolean {
    let pos = Method.methods["get_size"].call_1( str );
    return Method.methods["self_insert_bb"].call_3( str, pos, sup );
} );

Method.methods["self_insert_bb"].add_surdef( 2, [ OtWrapperString, "to_Number", "to_String" ], function( str, pos, sup ) : boolean {
    let npos = Method.methods["to_Number"].call_1( pos );
    let nsup = Method.methods["to_String"].call_1( sup );
    return str.rp._self_insert( npos, nsup );
} );

Method.methods["self_remove_bb"].add_surdef( 2, [ OtWrapperString, "to_Number", "to_Number" ], function( str, pos, len ) : boolean {
    let npos = Method.methods["to_Number"].call_1( pos );
    let nlen = Method.methods["to_Number"].call_1( len );
    return str.rp._self_remove( npos, nlen );
} );

Method.methods["self_ot_merge_o"].add_surdef( 2, [ OtWrapperString, "to_String" ], function( str: { rp: OtWrapperString }, oth ) : boolean {
    let sth = Method.methods["to_String"].call_1( oth );
    return str.rp._self_insert( str.rp.val.length.val, sth );
} );
Method.methods["self_ot_merge_o"].add_surdef( 3, [ OtWrapperString, OtWrapperString ], function( str: { rp: OtWrapperString }, oth: { rp: OtWrapperString }, as_usr : UsrId ) : boolean {
