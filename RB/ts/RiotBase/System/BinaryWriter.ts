...
export default class BinaryWriter {
    ...
    write_DevId( dev_id : DevId, cur_dev? : DevId ) {
        if ( dev_id.loc() && cur_dev != undefined )
            return this.write_DevId( cur_dev );
        this.write_some( Base64o.decode( dev_id.val ).reverse() );
    }
    ...
    write_UsrId( data : UsrId, cur_dev? : DevId, cur_usr? : UsrId ) : void {
        if ( ! data.num && cur_usr )
            return this.write_UsrId( cur_usr, cur_dev );
        this.write_PT( data.num );
        if ( data.num )
            this.write_DevId( data.dev, cur_dev );
    }

    write_GrpId( data : GrpId, cur_dev? : DevId ) : void {
        this.write_PT( data.num );
        if ( data.num )
            this.write_DevId( data.dev, cur_dev );
    }

    write_Inode( data : Inode, cur_dev? : DevId ) : void {
        this.write_PT( data.num );
        if ( data.num )
            this.write_DevId( data.dev, cur_dev );
    }

    write_PatchId( data : PatchId, cur_dev? : DevId ) : void{
        this.write_PT( data.num );
        if ( data.num )
            this.write_DevId( data.dev, cur_dev );
    }

    write_VectorClock( data : VectorClock, cur_dev? : DevId ) {
        this.write_PT( data.map.size );
        data.map.forEach( ( num, dev ) => {
            this.write_DevId( new DevId( dev ), cur_dev )
            this.write_PT( num )
        } );
        this.write_PT( data.merges.length );
        for( let inode of data.merges )
            this.write_Inode( inode, cur_dev );
    }
    ...
    data: ArrayBuffer;
    size: number        = 0;
    mark: Array<number> = []; // mark stack (cf this.beg_msg_length)
    i2rd: Array<number> = []; // int to reduce
}
