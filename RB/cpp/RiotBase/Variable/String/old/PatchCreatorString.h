...
///
class PatchCreatorString : public PatchCreator<PatchCreatorString> {
public:
    // operation types
    ...
    using OperationTypes = std::tuple<op_insert,op_remove,op_remove_und,op_new_cursor,op_set_cursor,op_set_cursor_und,op_rem_cursor,op_rem_cursor_und>;
    ...
    using RightTypes = std::tuple<can_read,can_insert,can_append,can_remove>;
    ...
    // methods
    PatchCreatorString( Rp *src, UsrId usr_id = {} );
    ~PatchCreatorString();
    ...
    // basic helpers
    CmString             _substring              ( PT pos, PT len ) const;
    PT                   _size                   () const;
    ...
    // operations
    using PatchCreator<PatchCreatorString>::apply_op;
    ...
    // rights
    using PatchCreator<PatchCreatorString>::right_to;
    ...
    // undo
    using PatchCreator<PatchCreatorString>::undo_patch;
    void                 undo_patch              ( op_insert        , Undo, PT pos, const CbString &str );
    void                 undo_patch              ( op_remove        , Undo, PT pos, PT len              );
    void                 undo_patch              ( op_remove_und    , Undo, PT pos, const CbString &str );
    void                 undo_patch              ( op_new_cursor    , Undo, DevId dev_id, PI64 loc_id, PT pos );
    void                 undo_patch              ( op_set_cursor    , Undo, DevId dev_id, PI64 loc_id, PT pos );
    void                 undo_patch              ( op_set_cursor_und, Undo, DevId dev_id, PI64 loc_id, PT pos, PT old_pos );
    void                 undo_patch              ( op_rem_cursor    , Undo, DevId dev_id, PI64 loc_id );
    void                 undo_patch              ( op_rem_cursor_und, Undo, DevId dev_id, PI64 loc_id, PT old_pos, UsrId usr );

    // transmission
    using PatchCreator<PatchCreatorString>::transmission;
    void                 transmission            ( op_remove_und    , BBQ br, const FqId &id, PT pos, CbString &old );
    void                 transmission            ( op_set_cursor_und, BBQ br, const FqId &id, DevId dev_id, PI64 loc_id, PT pos, PT old_pos );
    void                 transmission            ( op_rem_cursor_und, BBQ br, const FqId &id, DevId dev_id, PI64 loc_id, PT old_pos, UsrId usr );

    //
    using PatchCreator<PatchCreatorString>::unk_new_or_new_unk;
    ...
    //
    using PatchCreator<PatchCreatorString>::unk_std_comb;
    ...
    using PatchCreator<PatchCreatorString>::register_patch_op;
