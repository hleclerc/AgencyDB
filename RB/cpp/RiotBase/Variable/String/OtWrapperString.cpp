...
OtWrapperString::OtWrapperString( Rp *str, UsrId usr_id ) : OtWrapper<OtWrapperString>( usr_id ), str( str ), cur_cursor_id( 0 ) {
    RO_CALL( apply_DaSi, str, [ this ]( DaSi dasi ) { if ( dasi.size ) reg_op( op_insert{}, 0, dasi ); } );
}

OtWrapperString::~OtWrapperString() {
    RO_CALL( del, str );
}

std::string OtWrapperString::type_name() {
    return "OtWrapperString";
}

const Category *OtWrapperString::static_category() {
    return String::category;
}

const Rp *OtWrapperString::prox_ref() const {
    return str;
}

void OtWrapperString::write_structure( std::ostream &os ) const {
    RO_CALL( write_structure, str, os << type_name() << '(' ); os << ')';
}


Rp *OtWrapperString::new_cursor( void *int_id ) {
    return OtWrapperStringCursor::New( this, _new_cursor( 0, ++cur_cursor_id, 0, int_id ) );
}

void OtWrapperString::rem_cursor( void *int_id ) {
    for( AutoPtr<Cursor> &c : cursors )
        if ( c->int_id == int_id )
            return _rem_cursor( c.ptr() );
}

void OtWrapperString::get_cursors( Vec<CursorData> &res ) const {
    res.reserve( res.size() + cursors.size() );
    for( const AutoPtr<Cursor> &c : cursors )
        res.push_back( OtWrapperStringCursor::New( ccast( this ), ccast( c.ptr() ) ), c->dev_id, c->loc_id, c->usr_id, c->int_id );
}

void OtWrapperString::sig_change() {
    OtWrapper<OtWrapperString>::sig_change();
    RO_CALL( sig_change, str );
}

void OtWrapperString::_substring( PT pos, PT len, const std::function<void(DaSi)> &f ) const {
    RO_CALL( apply_DaSi, str, [ pos, len, &f ]( DaSi d ) { f( { d.data + std::min( pos, d.size ), std::min( pos + len, d.size ) - std::min( pos, d.size ) } ); } );
}

PT OtWrapperString::_size() const {
    PT res;
    RO_CALL( apply_DaSi, str, [ &res ]( DaSi d ) { res = d.size; } );
    return res;
}

_TCB void OtWrapperString::_insert( PT pos, const Cb &data ) {
    if ( not right_to( op_insert{}, {}, pos, data ) )
         return;
    reg_op( op_insert(), pos, data );
    Aod aod;
    apply_op( op_insert{}, aod, pos, data );
}

void OtWrapperString::_remove( PT pos, PT len ) {
    if ( not right_to( op_remove{}, {}, pos, len ) )
         return;

    // OT. IMPORTANT_TODO: utf8
    DaSi ds = RO_CALL( dasi, str );
    pos = std::min( pos, ds.size );
    len = std::min( len, ds.size - pos );
    reg_op( op_remove_und{}, pos, CmString( ds.beg() + pos, ds.beg() + pos + len ) );

    Aod aod;
    apply_op( op_remove(), aod, pos, len );
}

OtWrapperString::Cursor *OtWrapperString::_new_cursor_wo_cr( DevId dev_id, PI64 loc_id, PT pos, UsrId as_usr, void *int_id ) {
...
void OtWrapperString::_set_cursor_wo_cr( Cursor *cur, PT pos ) {
...
OtWrapperString::Cursor *OtWrapperString::_new_cursor( DevId dev_id, PI64 loc_id, PT pos, void *int_id ) {
    if ( not has_right( can_read{} ) )
        return 0;
    reg_op( op_new_cursor{}, dev_id, loc_id, pos );
    return _new_cursor_wo_cr( dev_id, loc_id, pos, {}, int_id );
}

void OtWrapperString::_set_cursor( OtWrapperString::Cursor *cur, PT pos ) {
    if ( not has_right( can_read{} ) or not cur or cur->pos == pos )
        return;
    reg_op( op_set_cursor_und{}, cur->dev_id, cur->loc_id, pos, cur->pos );
    _set_cursor_wo_cr( cur, pos );
}

void OtWrapperString::_rem_cursor( Cursor *cur ) {
    if ( not has_right( can_read{} ) or not cur )
        return;
    reg_op( op_rem_cursor_und{}, cur->dev_id, cur->loc_id, cur->pos, cur->usr_id );
    _rem_cursor_wo_cr( cur );
}

void OtWrapperString::_rem_cursor_wo_cr( Cursor *cur ) {
...
void OtWrapperString::apply_op( op_insert, Aod &aod, PT pos, const CbString &str ) {
    // cursors
    for( AutoPtr<Cursor> &c : cursors )
        if ( c->pos >= pos )
            c->pos += str.size();
    // op
    Number n( pos ); String s{ (std::string)str }; // TODO: optimize (e.g. by using a specialization of self_insert)
    RO_CALL( self_insert_bb, this->str, n.rp, s.rp );
    sig_change();
}

void OtWrapperString::apply_op( op_remove, Aod &aod, PT pos, PT len ) {
    // cursors
    for( AutoPtr<Cursor> &c : cursors ) {
        if ( c->pos >= pos + len )
            c->pos -= len;
    ...
    // op
    Number p( pos ), l( len ); // TODO: optimize
    RO_CALL( self_remove_bb, str, p.rp, l.rp );
    sig_change();
}

void OtWrapperString::apply_op( op_remove_und, Aod &aod, PT pos, const CbString &str ) {
    apply_op( op_remove(), aod, pos, str.size() );
}

void OtWrapperString::apply_op( op_new_cursor, Aod &aod, DevId dev_id, PI64 loc_id, PT pos ) {
    _new_cursor_wo_cr( dev_id, loc_id, pos, aod.as_usr, 0 );
}

void OtWrapperString::apply_op( op_set_cursor, Aod &aod, DevId dev_id, PI64 loc_id, PT pos ) {
    for( AutoPtr<Cursor> &c : cursors )
        if ( c->dev_id == dev_id and c->loc_id == loc_id )
            return _set_cursor_wo_cr( c.ptr(), pos );
}

void OtWrapperString::apply_op( op_set_cursor_und, Aod &aod, DevId dev_id, PI64 loc_id, PT pos, PT old_pos ) {
    apply_op( op_set_cursor{}, aod, dev_id, loc_id, pos );
}

void OtWrapperString::apply_op( op_rem_cursor, Aod &aod, DevId dev_id, PI64 loc_id ) {
    for( PT i = 0; i < cursors.size(); ++i ) {
...
void OtWrapperString::apply_op( op_rem_cursor_und, Aod &aod, DevId dev_id, PI64 loc_id, PT old_pos, UsrId usr ) {
    apply_op( op_rem_cursor{}, aod, dev_id, loc_id );
}


// transmission
void OtWrapperString::transmission( op_remove_und, BBQ br, PT pos, CbString &old ) {
    transmission( op_remove{}, br, pos, old.size() );
}

void OtWrapperString::transmission( op_set_cursor_und, BBQ br, DevId dev_id, PI64 loc_id, PT pos, PT old_pos ) {
    transmission( op_set_cursor{}, br, dev_id, loc_id, pos );
}

void OtWrapperString::transmission( op_rem_cursor_und, BBQ br, DevId dev_id, PI64 loc_id, PT old_pos, UsrId usr ) {
    transmission( op_rem_cursor{}, br, dev_id, loc_id );
}

// register (from a transmission)
void OtWrapperString::register_patch_op( op_remove, BBQ bq, PT pos, PT len ) {
    _substring( pos, len, [&]( DaSi d ) { reg_op_in( bq, OtWrapperString::op_remove_und(), pos, d ); } );
}

void OtWrapperString::register_patch_op( op_set_cursor, BBQ bq, DevId dev_id, PI64 loc_id, PT pos ) {
    PT old_pos = 0;
    for( AutoPtr<OtWrapperString::Cursor> &c : cursors ) {
        if ( c->dev_id == dev_id and c->loc_id == loc_id ) {
            old_pos = c->pos;
            break;
        }
    }
    reg_op_in( bq, OtWrapperString::op_set_cursor_und(), dev_id, loc_id, pos, old_pos );
}

void OtWrapperString::register_patch_op( op_rem_cursor, BBQ bq, DevId dev_id, PI64 loc_id ) {
    PT old_pos = 0;
    UsrId old_usr;
    for( AutoPtr<OtWrapperString::Cursor> &c : cursors ) {
        if ( c->dev_id == dev_id and c->loc_id == loc_id ) {
    ...
    reg_op_in( bq, OtWrapperString::op_rem_cursor_und(), dev_id, loc_id, old_pos, old_usr );
}

// undo
void OtWrapperString::undo_patch( op_insert, Aod &aod, PT pos, const CbString &dat ) {
    apply_op( op_remove{}, aod, pos, dat.size() );
}

void OtWrapperString::undo_patch( op_remove, Aod &aod, PT pos, PT len ) {
    WARNING( "Undo of a basic remove should not happen" );
}

void OtWrapperString::undo_patch( op_remove_und, Aod &aod, PT pos, const CbString &str ) {
    apply_op( op_insert{}, aod, pos, str );
}

void OtWrapperString::undo_patch( op_new_cursor, Aod &aod, DevId dev_id, PI64 loc_id, PT pos ) {
    apply_op( op_rem_cursor{}, aod, dev_id, loc_id );
}

void OtWrapperString::undo_patch( op_set_cursor, Aod &aod, DevId dev_id, PI64 loc_id, PT pos ) {
    WARNING( "Undoing a non und version should not happen" );
}

void OtWrapperString::undo_patch( op_set_cursor_und, Aod &aod, DevId dev_id, PI64 loc_id, PT pos, PT old_pos ) {
    apply_op( OtWrapperString::op_set_cursor(), aod, dev_id, loc_id, old_pos );
}

void OtWrapperString::undo_patch( op_rem_cursor, Aod &aod, DevId dev_id, PI64 loc_id ) {
    WARNING( "Undoing a non und version should not happen" );
}

void OtWrapperString::undo_patch( op_rem_cursor_und, Aod &aod, DevId dev_id, PI64 loc_id, PT old_pos, UsrId usr ) {
    apply_op( OtWrapperString::op_new_cursor(), aod, dev_id, loc_id, old_pos );
}

void OtWrapperString::unk_new_or_new_unk( op_insert, op_insert, UsrId asu_n, BBQ bq_n, BBQ bq_o, PT &p_n, CbString &d_n, PT &p_o, CbString &d_o ) {
    if ( p_o > p_n ) {
        // orig 01234
...
//
void OtWrapperString::unk_std_comb( op_insert, op_insert, UsrId asu_s, BBQ bq_u, PT &p_s, CbString &d_s, PT &p_u, CbString &d_u ) {
    if ( p_u > p_s ) {
        //  orig 012
...
void OtWrapperString::unk_std_comb( op_insert, op_remove, UsrId asu_s, BBQ bq_u, PT &p_s, CbString &d_s, PT &p_u, PT &l_u ) {
    if ( p_s > p_u ) {
        //  orig 012345678
...
void OtWrapperString::unk_std_comb( op_insert, op_remove_und, UsrId asu_s, BBQ bq_u, PT &p_s, CbString &d_s, PT &p_u, CbString &d_u ) {
    if ( p_s > p_u ) {
        //  orig 012345678
        //  +unk 045678     REM p_u=1,l_u=3
        //  +std 04b5678    INS p_s=2,d_s=b
...
void OtWrapperString::unk_std_comb( op_insert, op_new_cursor, UsrId asu_s, BBQ bq_u, PT &p_s, CbString &d_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u ) {
    if ( pos_u >= p_s ) {
        //  orig 012
...
void OtWrapperString::unk_std_comb( op_insert, op_set_cursor, UsrId asu_s, BBQ bq_u, PT &p_s, CbString &d_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u ) {
    if ( pos_u >= p_s ) {
        //  orig 012
        //  +unk 01|2  NEW_CURSOR pos=2
        //  +std 0b1|2 INS p_s=1,d_s=b
...
void OtWrapperString::unk_std_comb( op_insert, op_set_cursor_und, UsrId asu_s, BBQ bq_u, PT &p_s, CbString &d_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u, PT &old_pos_u ) {
    if ( pos_u >= p_s ) {
        //  orig 012
        //  +unk 01|2  NEW_CURSOR pos=2
        //  +std 0b1|2 INS p_s=1,d_s=b
...
void OtWrapperString::unk_std_comb( op_remove, op_insert, UsrId asu_s, BBQ bq_u, PT &p_s, PT &l_s, PT &p_u, CbString &d_u ) {
    if ( p_u > p_s ) {
        //  orig 012345678
...
void OtWrapperString::unk_std_comb( op_remove, op_remove, UsrId asu_s, BBQ bq_u, PT &p_s, PT &l_s, PT &p_u, PT &l_u ) {
    if ( p_s >= p_u ) {
        //  orig 012345678
...
void OtWrapperString::unk_std_comb( op_remove, op_remove_und, UsrId asu_s, BBQ bq_u, PT &p_s, PT &l_s, PT &p_u, CbString &d_u ) {
    if ( p_s >= p_u ) {
        //  orig 012345678
        //  +unk 0345678    REM p_u=1,l_u=2
        //  +std 03478      REM p_s=3,l_s=2
...
void OtWrapperString::unk_std_comb( op_remove, op_new_cursor, UsrId asu_s, BBQ bq_u, PT &p_s, PT &l_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u ) {
    if ( pos_u >= p_s + l_s ) {
        //  orig 012345
...
void OtWrapperString::unk_std_comb( op_remove, op_set_cursor, UsrId asu_s, BBQ bq_u, PT &p_s, PT &l_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u ) {
    if ( pos_u >= p_s + l_s ) {
        //  orig 012345
        //  +unk 01234|5  NEW_CURSOR pos=5
        //  +std 014|5    REM p_s=2,l_s=2
...
void OtWrapperString::unk_std_comb( op_remove, op_set_cursor_und, UsrId asu_s, BBQ bq_u, PT &p_s, PT &l_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u, PT &old_pos_u ) {
    if ( pos_u >= p_s + l_s ) {
        //  orig 012345
...
void OtWrapperString::unk_std_comb( op_remove_und, op_insert, UsrId asu_s, BBQ bq_u, PT &p_s, CbString &d_s, PT &p_u, CbString &d_u ) {
    if ( p_u > p_s ) {
        //  orig 012345678
        //  +unk 0123456b78 INS p_u=7,d_u=b
        //  +std 0456b78    REM p_s=1,l_s=3
...
void OtWrapperString::unk_std_comb( op_remove_und, op_remove, UsrId asu_s, BBQ bq_u, PT &p_s, CbString &d_s, PT &p_u, PT &l_u ) {
    if ( p_s >= p_u ) {
        //  orig 012345678
        //  +unk 0345678    REM p_u=1,l_u=2
        //  +std 03478      REM p_s=3,l_s=2
...
void OtWrapperString::unk_std_comb( op_remove_und, op_remove_und, UsrId asu_s, BBQ bq_u, PT &p_s, CbString &d_s, PT &p_u, CbString &d_u ) {
    if ( p_s >= p_u ) {
        //  orig 012345678
        //  +unk 0345678    REM p_u=1,l_u=2
        //  +std 03478      REM p_s=3,l_s=2
...
void OtWrapperString::unk_std_comb( op_remove_und, op_new_cursor, UsrId asu_s, BBQ bq_u, PT &p_s, CbString &d_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u ) {
...
void OtWrapperString::unk_std_comb( op_remove_und, op_set_cursor, UsrId asu_s, BBQ bq_u, PT &p_s, CbString &d_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u ) {
    if ( pos_u >= p_s + d_s.size() ) {
        //  orig 012345
        //  +unk 01234|5  NEW_CURSOR pos=5
        //  +std 014|5    REM p_s=2,l_s=2
...
void OtWrapperString::unk_std_comb( op_remove_und, op_set_cursor_und, UsrId asu_s, BBQ bq_u, PT &p_s, CbString &d_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u, PT &old_pos_u ) {
    if ( pos_u >= p_s + d_s.size() ) {
        //  orig 012345
        //  +unk 01234|5  SET_CURSOR pos=5
        //  +std 014|5    REM p_s=2,l_s=2
...
void OtWrapperString::unk_std_comb( op_set_cursor, op_new_cursor, UsrId asu_s, BBQ bq_u, DevId &dev_id_s, PI64 &loc_id_s, PT &pos_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u ) {
     if ( dev_id_s == dev_id_u and loc_id_s == loc_id_u )
         loc_id_u = 0;
}
void OtWrapperString::unk_std_comb( op_set_cursor, op_set_cursor, UsrId asu_s, BBQ bq_u, DevId &dev_id_s, PI64 &loc_id_s, PT &pos_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u ) {
    if ( dev_id_s == dev_id_u and loc_id_s == loc_id_u )
        loc_id_u = 0;
}
void OtWrapperString::unk_std_comb( op_set_cursor, op_set_cursor_und, UsrId asu_s, BBQ bq_u, DevId &dev_id_s, PI64 &loc_id_s, PT &pos_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u, PT &old_pos_u ) {
    if ( dev_id_s == dev_id_u and loc_id_s == loc_id_u )
        loc_id_u = 0;
}

void OtWrapperString::unk_std_comb( op_set_cursor_und, op_new_cursor, UsrId asu_s, BBQ bq_u, DevId &dev_id_s, PI64 &loc_id_s, PT &pos_s, PT &old_pos_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u ) {
    if ( dev_id_s == dev_id_u and loc_id_s == loc_id_u )
        loc_id_u = 0;
}
void OtWrapperString::unk_std_comb( op_set_cursor_und, op_set_cursor, UsrId asu_s, BBQ bq_u, DevId &dev_id_s, PI64 &loc_id_s, PT &pos_s, PT &old_pos_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u ) {
    if ( dev_id_s == dev_id_u and loc_id_s == loc_id_u )
        loc_id_u = 0;
}
void OtWrapperString::unk_std_comb( op_set_cursor_und, op_set_cursor_und, UsrId asu_s, BBQ bq_u, DevId &dev_id_s, PI64 &loc_id_s, PT &pos_s, PT &old_pos_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u, PT &old_pos_u ) {
    if ( dev_id_s == dev_id_u and loc_id_s == loc_id_u )
        loc_id_u = 0;
}

void OtWrapperString::unk_std_comb( op_rem_cursor, op_new_cursor, UsrId asu_s, BBQ bq_u, DevId &dev_id_s, PI64 &loc_id_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u ) {
    if ( dev_id_s == dev_id_u and loc_id_s == loc_id_u )
        loc_id_u = 0;
}
void OtWrapperString::unk_std_comb( op_rem_cursor, op_set_cursor, UsrId asu_s, BBQ bq_u, DevId &dev_id_s, PI64 &loc_id_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u ) {
    if ( dev_id_s == dev_id_u and loc_id_s == loc_id_u )
        loc_id_u = 0;
}
void OtWrapperString::unk_std_comb( op_rem_cursor, op_set_cursor_und, UsrId asu_s, BBQ bq_u, DevId &dev_id_s, PI64 &loc_id_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u, PT &old_pos_u ) {
    if ( dev_id_s == dev_id_u and loc_id_s == loc_id_u )
        loc_id_u = 0;
}

void OtWrapperString::unk_std_comb( op_rem_cursor_und, op_new_cursor, UsrId asu_s, BBQ bq_u, DevId &dev_id_s, PI64 &loc_id_s, PT &old_pos_s, UsrId usr_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u ) {
    if ( dev_id_s == dev_id_u and loc_id_s == loc_id_u )
        loc_id_u = 0;
}
void OtWrapperString::unk_std_comb( op_rem_cursor_und, op_set_cursor, UsrId asu_s, BBQ bq_u, DevId &dev_id_s, PI64 &loc_id_s, PT &old_pos_s, UsrId usr_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u ) {
    if ( dev_id_s == dev_id_u and loc_id_s == loc_id_u )
        loc_id_u = 0;
}
void OtWrapperString::unk_std_comb( op_rem_cursor_und, op_set_cursor_und, UsrId asu_s, BBQ bq_u, DevId &dev_id_s, PI64 &loc_id_s, PT &old_pos_s, UsrId usr_s, DevId &dev_id_u, PI64 &loc_id_u, PT &pos_u, PT &old_pos_u ) {
    if ( dev_id_s == dev_id_u and loc_id_s == loc_id_u )
        loc_id_u = 0;
}
...
RiotBase_DEF_FUNC_2( self_add_b, 1, tv0->isa<OtWrapperString>() and tv1->has_surdef_for( RF_dasi() ) ) {
    OtWrapperString *p = scast( v0 );
    DaSi ds0 = RO_CALL( dasi, p->str );
    DaSi ds1 = RO_CALL( dasi, v1 );
    p->_insert( ds0.size, CmString( ds1.beg(), ds1.end() ) );
}

RiotBase_DEF_FUNC_2( self_set_b, 1, tv0->isa<OtWrapperString>() and tv1->has_surdef_for( RF_dasi() ) ) {
    if ( reinterpret_cast<String &>( v0 ).size() )
        TODO;
    RO_CALL( self_add_b, v0, v1 );
}

RiotBase_DEF_FUNC_2( self_set_o, 1, tv0->isa<OtWrapperString>() and tv1->has_surdef_for( RF_dasi() ) ) {
    RO_CALL( self_set_b, v0, v1 );
    RO_CALL( del, v1 );
}

RiotBase_DEF_FUNC_3( self_insert_bb, 1, tv0->isa<OtWrapperString>() and tv1->has_surdef_for( RF_to_PT() ) and tv2->has_surdef_for( RF_dasi() ) ) {
...
RiotBase_DEF_FUNC_3( self_remove_bb, 1, tv0->isa<OtWrapperString>() and tv1->has_surdef_for( RF_to_PT() ) and tv2->has_surdef_for( RF_to_PT() ) ) {
    PT pos; if ( not RO_CALL( to_PT, v1, &pos, 1 ) ) return;
