...
struct UsrId : Inode {
    ...
    template<class Cq>
    UsrId( Hpipe::BinStream<Cq> &bq ) : Inode( bq ) {}

    template<class Cq>
    UsrId( Hpipe::BinStream<Cq> &bq, DevId cur_dev, UsrId cur_usr ) : Inode( bq, cur_dev ) { if ( *this == cur_usr ) num = 0; }
    ...
    template<class Cq>
    static UsrId read_from    ( Hpipe::BinStream<Cq> &bq ) { PI64 num = bq.read(); DevId dev; if ( num ) dev = bq.read(); return { dev, num }; }
