        ...
        case 2: { // Insert
            ...
            while ( br_unk.size ) {
                ...
                switch ( num_unk ) {
                    ...
                    case 3: { // Remove
                        let pos_unk = br_unk.read_PT(), len_unk = br_unk.read_PT();
                        if(!(pos_new<pos_unk+len_unk)){pos_new-=len_unk;}else{if(pos_new<=pos_unk){pos_unk+=str_new.length;}else{pos_new=pos_unk;lst_bw.push(3);pos_unk+=str_new.length;len_unk-=pos_new-pos_unk;lst_bw.push(pos_unk);lst_bw.push((pos_new-pos_unk));}}
                        bw_unk.write_PI8( 3 ); bw_unk.write_PT( pos_unk ); bw_unk.write_PT( len_unk );
                        break;
                    }
                    case 4: { // RemUnd
